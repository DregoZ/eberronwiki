  ...
  template: `
    <aside class="sidebar">
      <div class="sidebar-header">
        <a routerLink="/" class="logo-link" (click)="linkClick.emit()">
      ...
      <div class="sidebar-nav">
        @if (navService.isLoading()) {
          ...
          @for (node of navService.navTree(); track node.id) {
            <app-sidebar-node [node]="node" (linkClick)="linkClick.emit()" />
          }
        }
      </div>
    </aside>
...
export class SidebarComponent {
  ...
  // Emite al padre (MainLayoutComponent) para cerrar mat-sidenav en móvil
